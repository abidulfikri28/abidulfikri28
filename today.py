...
def get_avatar_ascii(username, width=47, height=25):
    ascii_chars = '@$#&WM8BbdqoOQLCJzxrjft/\\|(){}?-_+~Il;:,". '
    try:
        url = f"https://avatars.githubusercontent.com/{username}?size=460"
        response = requests.get(url, headers=HEADERS, timeout=10)
        if response.status_code != 200:
            raise Exception(f"Avatar fetch failed: {response.status_code}")
        img = Image.open(BytesIO(response.content))
        img = img.convert('L')
        w, h = img.size
        # Remove circular border, then crop to face+shoulder region
        margin = int(w * 0.10)
        inner = img.crop((margin, margin, w - margin, h - margin))
        face = inner.crop((0, 0, inner.width, int(inner.height * 0.55)))
        face = ImageOps.autocontrast(face, cutoff=2)
        face = face.filter(ImageFilter.SHARPEN)
        face = face.point(lambda p: 255 if p > 195 else p)
        ...
        print(f"ASCII art generation failed: {e}")
        return [' ' * width] * height

def update_ascii_art(root, ascii_lines):
    for i, line in enumerate(ascii_lines):
        element = root.find(f".//*[@id='ascii_r{i}']")
...
def formatter(query_type, difference):
    print('{:<25}'.format(' ' + query_type + ':'), sep='', end='')
...
if __name__ == '__main__':
    ...
    ascii_lines, ascii_time = perf_counter(get_avatar_ascii, USER_NAME)
    formatter('avatar ascii art', ascii_time)
    ...
    svg_overwrite('dark_mode.svg', commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1], ascii_lines)
    svg_overwrite('light_mode.svg', commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1], ascii_lines)
